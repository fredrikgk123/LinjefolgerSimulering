# main.py — sets random seed before any other import (matplotlib may call numpy RNG).
import numpy as np
import sys
import os
...
def main():
    ...
    parser.add_argument("--track", type=str, default="../assets/bane_fase2.png",
                        help="Track image in assets/ (default: bane_fase2.png)")
    ...
    speed_controller = SpeedController(
        ...
        turn_factor_threshold=SC_TURN_FACTOR_THRESHOLD,
        min_speed_factor=SC_MIN_SPEED_FACTOR)
    ...
    if cp_total:
        print(f"Checkpoints: {cp_total} must be cleared in order.")
    else:
        print("No checkpoints defined — lap valid via start/finish zone only.")

    update_plot, _ = setup_realtime_plot(
        blur_arr, spawn=(x_start, y_start),
        sf_radius=START_FINISH_RADIUS, checkpoints=checkpoints)
    ...
    lap_time       = None
    lap_start_t    = 0.0
    in_finish_zone = False   # debounce: True while inside START_FINISH_RADIUS
    ...
    while t < MAX_LAP_TIME:
        ...
        e_norm = e_y / QTR_HALF_SPAN   # normalised error ±1

        # Control
        w_cmd = pid.compute(e_norm, DT)
        v_cmd = speed_controller.update(e_norm, w_cmd, pid.limit)
        ...
        # Lap timer (edge-triggered on start/finish zone entry)
        if not lap_started and dist_to_start > MIN_DEPARTURE_DIST:
            lap_started    = True
            lap_start_t    = t
            in_finish_zone = False

        if lap_started and lap_time is None:
            if dist_to_start < START_FINISH_RADIUS:
                if not in_finish_zone:
                    in_finish_zone = True
                    if cp_next < cp_total:
                        print(f"  *** Passed start/finish — only {cp_next}/{cp_total} "
                              f"checkpoints cleared. Continuing... ***")
                    else:
